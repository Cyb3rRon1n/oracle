...
- **Clients** (`client/`, Textual TUI): thin — render the character sheet pane (including active objectives) + narrative/input log, send player actions to the engine. Hotseat and networked play are the same client/engine pair with different transports (local I/O vs. sockets).
