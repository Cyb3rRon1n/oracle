...
from .portrait import DEFAULT_STYLE, build_portrait_prompt, build_scene_prompt
from .rolls import (
    DEFAULT_NPC_HP,
...
class GameEngine:
    ...
    async def _on_generate_scene(self, envelope: Envelope) -> None:
        """Player-initiated scene-banner generation - world-level art, not
        tied to one character, so any joined player may trigger it (same
        "any joined player" shape as start_combat) rather than gating on the
        sender owning a character. Same manual-only, "always asks first"
        stance as generate_portrait - no automatic regeneration on location/
        mood change, only a clear of the stale image (see WorldState.
        apply_update). Exempt from turn order. Repeatable."""
        player_id = envelope.sender_id
        if self._image_backend is None:
            await self._send_to(
                player_id,
                self._system_envelope("Scene generation isn't configured on this server.", level="warning"),
            )
            return

        style = envelope.payload.get("style", DEFAULT_STYLE)
        prompt = build_scene_prompt(self._session.world, style=style)

        async def on_progress(step: int, total: int) -> None:
            await self._send_to(player_id, self._scene_progress_envelope(step, total))

        try:
            image_bytes = await self._image_backend.generate_portrait(prompt, on_progress=on_progress)
        except Exception:
            logger.exception("Scene generation failed for session_id=%s", self._session.session_id)
            await self._send_to(
                player_id, self._system_envelope("Couldn't generate a scene image right now.", level="warning")
            )
            return

        self._session.world.scene_image = f"data:image/png;base64,{base64.b64encode(image_bytes).decode()}"
        # Broadcast, not private - a scene banner is shared party-visible
        # fiction like location/mood, same reasoning _world_update_envelope
        # already documents.
        await self._broadcast(self._world_update_envelope())
        await self._save()
    ...
    def _scene_progress_envelope(self, step: int, total: int) -> Envelope:
        return Envelope(
            type="scene_progress",
            session_id=self._session.session_id,
            sender_id="server",
