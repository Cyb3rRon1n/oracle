...
| type | payload | purpose |
|---|---|---|
| `state_sync` | `{characters[], npcs[], world_state, turn_order, current_turn, log_tail[]}` | full snapshot on join/reconnect — the recipient's own `characters[]` entry is a full sheet, every other player's entry is the same redacted public view (below) `player_joined`/`player_update` broadcast |
