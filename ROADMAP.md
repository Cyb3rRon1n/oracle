...
- **A real narration round-trip has been verified**, live, against `llama3.1:8b` running locally via Ollama — not just mocked. Full turn observed working: streamed narration text, a real `update_character` tool call (the model applied `hp_delta: -1` on its own judgment mid-narration), the character sheet updating to match, and the whole exchange persisting correctly to `sessions/default.json` across the resulting turn. CPU-only inference (no GPU on the dev machine) is slow — roughly 30-90s per turn — which is the main practical limitation of this backend right now. The hosted Claude backend (`AnthropicNarrator`) shares the identical engine/tool-loop code path but hasn't been run live yet — still pending account credits.
...
2. **Verify the hosted Claude path live too**, once there's a way to (credits, or just to compare narration quality against the local model side by side) — the code path is identical to Ollama's, so this is lower urgency than it was before local verification existed, but still worth doing since Claude and a local 8B model are not equally reliable at things like consistent tool-calling.
...
- **Tune the Ollama backend** — try other local models for narration quality/tool-call reliability tradeoffs now that the harness exists to compare them; consider whether the local model needs stronger prompting to only call `update_character` when narration text actually implies a mechanical change (the first live test saw it apply a small HP change without any injury being narrated).
