...
- CI, a 27-test suite, MIT license (with the SRD data separately CC-BY-4.0 attributed), issue/PR templates.
...
1. **Give the DM real memory.** Right now each turn's prompt (`server/engine.py._on_player_action`) only passes the *previous* turn's narration text as `world_summary` — there's no accumulated history of earlier turns, NPCs introduced, or plot threads. Past 2-3 turns, the DM will plausibly start contradicting itself or forgetting things it said. This needs a real decision, not a quick patch:
   - Full conversation history per turn (simplest correctness-wise; lean on prompt caching to keep cost sane over a long session), or
   - A rolling window of the last N turns, or
   - An evolving structured world-state summary the DM explicitly rewrites each turn (more work, but bounds token growth better).

   This changes the shape of `NarratorBackend.narrate()` and how `Session` stores history, so it's worth deciding deliberately rather than building more on top of the current single-turn-context signature.

2. **Make narrated outcomes actually change the character sheet.** HP and inventory are set once at character creation and never touched again — the DM can *say* "you take 4 damage," but nothing enforces it. There's no real stakes or persistent consequence yet, just narration. Given the tool-use architecture already in place, the natural fix is an `update_character` tool (HP delta, add/remove inventory, add/remove a condition) the DM calls after narrating an outcome, which the engine applies and then emits as `character_update` to the player plus a `log_entry` to the room — `character_update` is already defined in `docs/protocol.md` and currently unused.

3. **Verify a real narration round-trip.** Either add Anthropic credits, or build a local-model `NarratorBackend` (the swap point already exists — see `create_narrator()` in `server/narrator.py`). Right now the entire pipeline up to the API call is tested and confirmed correct; nobody has actually seen the DM speak yet.
