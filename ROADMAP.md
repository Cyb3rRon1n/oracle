...
- **A real narration round-trip has been verified**, live, via Ollama — not just mocked, across four real turns total, two models compared head to head on the *same session* (same character, same rolling history):
...
3. **`qwen2.5:7b`'s 2-for-2 result needs a bigger sample before it's trusted.** If a future session sees it fail to call `update_character` when it should (or call it when it shouldn't), that's worth logging here the same way the `llama3.1:8b` failure was — don't just quietly patch it. If it keeps checking out, this item can be closed.
