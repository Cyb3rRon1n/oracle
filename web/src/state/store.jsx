...
function initial() {
  return {
    ...
    world: { location: "", summary: "", mood: "", scene_image: "", flags: {}, objectives: [], map: { nodes: [], edges: [] }, clocks: [] },
    turnOrder: [],
    currentTurn: null,
    inCombat: false,
    log: [],
    ...
    scenePending: false, // true between requesting a scene banner and the result (a world_update, or a warning) coming back
    sceneProgress: null, // {step, total} while a local ComfyUI backend reports real per-step progress; null otherwise
...
function reducer(state, event) {
  switch (event.type) {
    ...
    case ET.WORLD_UPDATE:
      return {
        ...state,
        world: { ...state.world, ...event.payload },
        scenePending: false,
        sceneProgress: null,
      };
    ...
    case ET.SYSTEM_MESSAGE:
      return {
        ...
        // Same rule for a scene-banner request that came back a warning
        // ("not configured", "couldn't generate") instead of a real update.
        scenePending: state.scenePending && event.payload.level === "info",
        sceneProgress: state.scenePending && event.payload.level === "info" ? state.sceneProgress : null,
    ...
    case "scene_pending":
      return { ...state, scenePending: true, sceneProgress: null };

    case ET.SCENE_PROGRESS:
      return { ...state, sceneProgress: { step: event.payload.step, total: event.payload.total } };
...
export function StoreProvider({ children }) {
  ...
  const actions = useMemo(
    () => ({
      ...
      generateScene(style) {
        connRef.current?.sendEvent(ET.GENERATE_SCENE, { style });
        dispatch({ type: "scene_pending" });
      },
      deathSave() {
